...
export const rolesModule = () => {
    ...
    router.delete("/:id/roles/:roleId", auth.handle.bind(auth),
        rolePerm.check("MANAGE_ROLES"), controller.deleteServerRole.bind(controller) );
    router.get("/:id/roles", auth.handle.bind(auth), controller.getServerRoles.bind(controller) );
    router.post("/:id/roles", auth.handle.bind(auth), 
        rolePerm.check("MANAGE_ROLES"),controller.createServerRole.bind(controller) );
    
    router.post("/:id/members/:userId/roles/:roleId", auth.handle.bind(auth), 
        rolePerm.check("MANAGE_ROLES"),controller.assignRoleToMember.bind(controller) );
    router.delete("/:id/members/:userId/roles/:roleId", auth.handle.bind(auth), 
        rolePerm.check("MANAGE_ROLES"),controller.unAsignRoleToMember.bind(controller) );
    ...
    router.patch("/roles/:roleId/permissions", auth.handle.bind(auth), 
        rolePerm.check("MANAGE_ROLES"), controller.updateRolePermissions.bind(controller) );
