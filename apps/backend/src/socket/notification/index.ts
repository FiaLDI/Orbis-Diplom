...
async function broadcastPresence(ioNotification: Namespace, userId: number, isOnline: boolean) {
  ...
  ioNotification.to(`friends_of_${userId}`).emit(isOnline ? "user-online" : "user-offline", payload);
  ...
  chatUsers.forEach((c) =>
    ioNotification.to(`chat_${c.chat_id}`).emit(isOnline ? "user-online" : "user-offline", payload)
  );
  ...
  servers.forEach((s) =>
    ioNotification.to(`server_${s.server_id}`).emit(isOnline ? "user-online" : "user-offline", payload)
  );
}
