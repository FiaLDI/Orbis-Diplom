...
import { useCreateMessagesMutation, useEditMessageMutation, useLazyGetMessagesQuery } from "../../api/chatApi";
import { useAppDispatch, useAppSelector } from "@/app/hooks";
import { leaveEditMode, sendMessageVisual, setReply, setUploadState } from "../../chatSlice";
...
const InputChat: React.FC<InputChatProps> = ({ scrollToBottom }) => {
    ...
    const handleEditClick = () => {
        editMessage({chat_id: isOpen?.chat_id, id: isOpen?.id, content: editMessagee });
        ...
        <div className="bg-[#2040b6] flex justify-between items-center shrink-0">
            {(editMode?.enabled && editMode.chatId == activeChat?.chat_id ) ? <>
                <input 
                    ...
                    value={editMessagee}
                    onChange={
                        (e) => seteditMessagee(e.target.value)
                    }
                    placeholder="Введите сообщение..."
                    onKeyPress={
                        (e) => e.key === "Enter" && handleEditClick()
                    }
                />
                <button onClick={handleEditClick} className="p-2 hover:brightness-90 cursor-pointer">
