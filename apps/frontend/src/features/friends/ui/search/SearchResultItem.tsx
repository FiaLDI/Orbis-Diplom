...
export const SearchResultItem: React.FC<Props> = ({ user, onMessage, onInvite, t }) => (
    <li className="flex gap-10 bg-[#4a55e9] p-3 justify-between rounded">
        ...
        <div className="flex gap-5">
            <ModalButton handler={() => onMessage(user.id)}>{t("modal.message")}</ModalButton>
            <ModalButton handler={() => onInvite(user.id)}>{t("modal.addfriend")}</ModalButton>
        </div>
    </li>
);
