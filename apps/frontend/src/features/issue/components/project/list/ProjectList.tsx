...
import { useProjectListModel } from "@/features/issue/model/useProjectListModel";
...
export const ProjectList: React.FC<{
    ...
    const { projects, handleProject, handlerCreateProject, open } = useProjectListModel(serverId);
    ...
    const header = (
        <div className="bg-background p-5 flex w-full justify-between items-center">
            <h4 className="truncate text-lg text-white w-full flex items-center gap-5">
                <button className="cursor-pointer" aria-label="Back" onClick={handleProject}>
                    <MoveLeft />
                </button>
                Projects {name}
            </h4>
