...
import { issueState } from "@/features/issue/types";
import { issueApi } from "@/features/issue/api";
import { findIssueById, flattenIssues } from "@/features/issue/utils";
...
const issueSlice = createSlice({
    ...
    reducers: {
        ...
        setChatName(state, action: PayloadAction<{ id: string; name: string }>) {
            const chat = state.issues?.chats?.find((c: any) => c.id === action.payload.id);
            if (chat) {
    ...
    extraReducers(builder) {
        builder
            ...
            .addMatcher(issueApi.endpoints.getChatIssue.matchFulfilled, (state, action) => {
                ...
                if (!issue) {
                    console.warn(`⚠️ issue ${issueId} не найден в state.issues`, state.issues);
                    return;
                }
...
export const { setOpenProject, setOpenIssue, toggleIssueMode, setChatName } = issueSlice.actions;
