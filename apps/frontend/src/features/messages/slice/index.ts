...
const messagesSlice = createSlice({
    ...
    reducers: {
        ...
        addMessage(state, action: PayloadAction<Message>) {
            ...
            const chatId = String(msg.chatId ?? (msg as any).chat_id ?? state.activeChat?.id ?? "");
            if (!chatId) return;
        ...
        updateMessageInHistory(state, action: PayloadAction<Message>) {
            ...
            const chatId = String((updated as any).chatId ?? (updated as any).chat_id ?? "");

            const patch = (arr: Message[]) => arr.map((m) => (m.id === updated.id ? updated : m));
