...
export const BannedSection: React.FC<{
    ...
    emptyLabel: string;
}> = ({ title, refreshingLabel, loading, items, onRefresh, onUnban, emptyLabel }) => (
    <Section>
        <SectionHeader
            title={title}
            action={
        ...
        {!items?.length && !loading && <div className="opacity-60">{emptyLabel}</div>}
        <div className="max-h-[300px] overflow-y-auto scroll-hidden">
            {items?.map((ban) => (
                <BannedRow key={ban.user_id} item={ban} onUnban={() => onUnban(ban.targetId)} />
            ))}
        </div>
    </Section>
);
