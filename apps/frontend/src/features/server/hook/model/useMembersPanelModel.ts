...
import { useLazyGetInfoUserQuery } from "@/features/user/api";
import {
    useBanUserMutation,
    useKickUserMutation,
...
import { useEmitServerUpdate } from "../emit";
...
export function useMembersPanelModel() {
    ...
    const myRoles = roles.filter((r: any) => r.members?.some((m: any) => m.userId === meId));

    const myServerPermissions: string[] = Array.from(
        new Set(myRoles.flatMap((r: any) => r.permissions ?? []))
    );

    const canKick = myServerPermissions.includes("KICK_USERS");
    const canBan = myServerPermissions.includes("BAN_USERS");
    ...
    const onViewProfile = useCallback((id: string) => triggerProfile(id), [triggerProfile]);
    ...
    const doKick = useCallback(
        async (userId: string) => {
            ...
            await kickUser({ serverId: activeserverId, userId })
                .unwrap()
                .catch(() => {});
            emitServerUpdate("members", activeserverId);
        },
