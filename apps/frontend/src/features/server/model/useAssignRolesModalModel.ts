...
import { useAssignRoleToMemberMutation, useRemoveRoleFromMemberMutation } from "@/features/server";
...
export function useAssignRolesModalModel(
    ...
    const onSubmit = async (data: { roles: string[] }) => {
        const toAdd = data.roles.filter((id) => !userRoles.some((r) => r.id === id));
        const toRemove = userRoles.map((r) => r.id).filter((id) => !data.roles.includes(id));
