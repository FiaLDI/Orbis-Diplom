...
const serverSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(
        serverApi.endpoints.GetServersChats.matchFulfilled,
        (state, action) => {
          if (!state.activeserver) return;
          state.activeserver = {
            ...state.activeserver,
      ...
      .addMatcher(
        ...
        (state, action) => {
          ...
          state.activeserver.roles = state.activeserver.roles.map(
            (role: any) =>
      ...
      .addMatcher(
        serverApi.endpoints.getLinks.matchFulfilled,
        (state, action) => {
          if (state.activeserver) {
            state.activeserver.inviteLinks = action.payload;
          }
        },
      );
  },
